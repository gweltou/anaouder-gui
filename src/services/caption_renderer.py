...
class CaptionRenderer:
    ...
    def __init__(self, document_controller: DocumentController) -> None:
        ...
        self.loaded_fonts = {
            ("default", self.DEFAULT_FONT_SIZE): ImageFont.load_default(
    ...
    def _set_document(self, document_controller: DocumentController) -> None:
        ...
        for block in document_controller.getAllBlocks():
            ...
            # Alignment data
            auto_transcription = self.document_controller.getTranscriptionForSegment(
                segment[0], segment[1]
            )
    ...
    def set_background_images(self, filepath_pattern: str) -> None:
        ...
        if match:
            ...
            p = (
                self.document_controller.document_path.parent
                if self.document_controller.document_path
                else Path()
            )
    ...
    def get_background_image(
        self, frame_number: int, segment_ids: List[SegmentId]
    ) -> Image.Image:
        ...
        if "background-color" in properties:
            ...
            if bg_color != self.background_color:
                ...
                self.empty_frame = Image.new(
                    "RGBA", self.frame_size, self.background_color
                )
    ...
    def render_colored_text(
        ...
        text_bg_outline_color = properties.get(
            "bg-outline-color", self.DEFAULT_FONT_OUTLINE_COLOR
        )
        text_bg_outline_width = int(
        ...
        for line in words_lines:
            ...
            for word in line:
                ...
                rendered_word = self.get_colored_word(
                    ...
                    text_bg_color,
                    text_fg_color,
                    text_bg_outline_color,
                    text_bg_outline_width,
                    text_fg_outline_color,
                    text_fg_outline_width,
                    get_word_progress(
                        segment_prog,
                        accumulated_chars,
                        total_chars,
        ...
        right += rendered_words[longest_i][-1][1][
            2
        ]  # Add the last word size, stroke weight included
    ...
    def get_colored_word(
        ...
        if not bg_img and color_pc < 1.0:
            # First layer, background text
            bg_img = Image.new(
                "RGBA", (text_img_width, text_img_height), self.background_color
            )
...
class VideoBurnerWorker(TaskWorker):
    def __init__(
        ...
        if self.video_source_path:
            ...
            self.media_duration = renderer.document_controller.getSortedSegments()[-1][
                1
            ][1]
    ...
    def run(self):
        ...
        ffmpeg_cmd = [
            ...
            "-hide_banner",
            "-progress",
            "pipe:1",  # write progress to stdout
            "-nostats",
            self.output_path,
        ]
