...
class DocumentController(QObject):
    ...
    def addSegment(
        self, segment: Segment, segment_id: Optional[SegmentId] = None
    ) -> SegmentId:
    ...
    def getSegmentsAtTime(
        self, position_sec: float, onset=0.0, offset=0.0
    ) -> List[SegmentId]:
    ...
    def getSegmentsAtTimeOffsets(
        self, position_sec: float, offsets: Dict[SegmentId, Tuple]
    ) -> List[SegmentId]:
    ...
    def getTranscriptionForSegment(
        ...
        cached_transcription = (
            cache.get_media_transcription(self.media_path) if self.media_path else None
        )
        ...
        if cached_transcription:
            if seg_end <= cached_transcription[-1][1]:
                ...
                try:
                    ...
                    logger.warning(
                        app_strings.TR_CANT_SMART_SPLIT + f": {e}", show_status=True
                    )
    ...
    def splitFromWaveform(self, segment_id: SegmentId, timepos: float) -> None:
        ...
        if cached_transcription:
            if seg_end <= cached_transcription[-1][1]:
                ...
                try:
                    logger.debug("Smart splitting")
                    left_text, right_text = smart_split_time(
                        text, timepos, tokens_range
                    )
                except Exception as e:
                    logger.error(app_strings.TR_CANT_SMART_SPLIT + f": {e}")
    ...
    def splitUtterance(
        ...
        self.undo_stack.push(
            DeleteUtterancesCommand(
                self, self.text_widget, self.waveform_widget, [seg_id]
            )
        )
        self.undo_stack.push(
            AddSegmentCommand(self, self.waveform_widget, left_seg, left_id)
        )
        ...
        self.undo_stack.push(
            AddSegmentCommand(self, self.waveform_widget, right_seg, right_id)
        )
        self.undo_stack.push(
            InsertBlockCommand(
                self,
        ...
        self.undo_stack.push(MoveTextCursor(self.text_widget, cursor.position()))
    ...
    def joinUtterances(self, segment_ids) -> None:
        ...
        self.undo_stack.push(
            JoinUtterancesCommand(
                self, self.text_widget, self.waveform_widget, segment_ids
            )
        )
    ...
    def deleteSegments(self, segments_id: List[SegmentId]) -> None:
        ...
        self.undo_stack.push(
            DeleteSegmentsCommand(
                self, self.text_widget, self.waveform_widget, segments_id
            )
        )
