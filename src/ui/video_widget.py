...
from PySide6.QtCore import QMargins, QPointF, QRectF, QSize, Qt, QTimer
...
class VideoWidget(QGraphicsView):
    def __init__(self, parent=None):
        ...
        self.background_rect_visible = app_settings.value(
            "subtitles/rect_visible", True, type=bool
        )
        ...
        self.text_item.setDefaultTextColor(
            app_settings.value("subtitles/font_color", QColor(255, 255, 255))
        )
        self.background_rect.setBrush(
            app_settings.value("subtitles/rect_color", QColor(0, 0, 0, 100))
        )
    ...
    def onMediaStatusChanged(self, status):
        ...
        if status in [QMediaPlayer.MediaStatus.BufferedMedia]:
            QTimer.singleShot(
                150, self.updateLayout
            )  # Small delay to ensure video size is available
