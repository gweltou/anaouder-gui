...
import pytz
import srt
from ostilhou.asr.dataset import METADATA_PATTERN, MetadataParser
...
def export_to_text_format(
    parent: QWidget, media_path: Optional[str], utterances: List[tuple], file_type: str
) -> None:
    """Open the export dialog and call the right
    export function depending of the file type

    Args:
        file_type (str): "srt", "txt" or "eaf"
    """
    ...
    try:
        ...
        logger.message(
            QObject.tr("Exported '{path}'").format(path=os.path.basename(file_path))
        )
    except IOError as e:
...
def format_srt(utterances: List[tuple]) -> str:
    ...
    subs = []
    for i, (text, segment) in enumerate(utterances):
        # data =
        # if data is None:
        #     continue
        regions, _ = metadata_parser.parse_sentence(text)
