...
from src.utils import LINE_BREAK, MEDIA_FORMATS, extract_metadata
...
class FileManager(QObject):
        ...
        try:
            with file_path.open("w", encoding="utf-8") as _fout:
                ...
                for text, segment in data:
                    ...
                    if media_path:
                        match = re.search(
                            r"{\s*(media|audio)\-path\s*:\s*(.*?)\s*}", text
                        )
                        if match:
                ...
                for line in _fin:
                    ...
                    if not media_path:
                        if "media-path" in metadata:
                            media_path = (
                                filepath.parent / metadata["media-path"]
                            ).resolve()
                        elif "audio-path" in metadata:
                            media_path = (
                                filepath.parent / metadata["audio-path"]
                            ).resolve()
    ...
    def _parse_srt_file(self, filepath: str) -> List:
        ...
        for encoding in encodings:
            try:
                with open(filepath, "r", encoding=encoding) as f_in:
                    ...
                    logger.debug(
                        f"Successfully parsed {len(subtitles)} subtitles using {encoding} encoding"
                    )
                    break
        ...
        for subtitle in subtitles:
            ...
            if start >= end:
                logger.debug(
                    f"Skipping invalid subtitle {subtitle.index}: start >= end"
                )
                continue
    ...
    def read_split_file(self, filepath: Path):
        ...
        with txt_filepath.open("r", encoding="utf-8") as _f:
            ...
            for sentence in _f.readlines():
                cleaned, _ = extract_metadata(sentence)
        ...
        return {"document": data, "media-path": str(media_path) if media_path else None}
    ...
    def get_backup_parent(self, filepath: Path) -> Optional[Path]:
        ...
        if filepath.parent.name == AUTOSAVE_FOLDER_NAME and bool(
            re.search(timestamp + r"$", filepath.stem)
        ):
